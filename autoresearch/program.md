...
4. **Run `prep_benchmark.py` first** if `benchmark_aligned.csv` doesn't exist:
   ```bash
   python autoresearch/prep_benchmark.py PhaseII_TestD_0003_h_01.csv
   ```
...
8. **Chart** — `python autoresearch/progress.py` (regenerates `autoresearch/progress.png`)
9. **Log** — Append to `results.tsv` (tab-separated, do NOT commit this file):
   ```
...
11. **Repeat** — Do NOT ask "should I continue?" The answer is always yes.
